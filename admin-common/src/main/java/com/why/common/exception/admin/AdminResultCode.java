...
@AllArgsConstructor
@ToString
public enum AdminResultCode implements ErrorStatus {
    /**
     * 400
     */
    ...
    public static AdminResultCode getBaseResoultCode(String code){
        AdminResultCode[] values = AdminResultCode.values();
        for (AdminResultCode value : values) {
            if(value.name().equalsIgnoreCase(code)){
                return value;
            }
        }
        return AdminResultCode.NOT_FOUND;
    }
}
