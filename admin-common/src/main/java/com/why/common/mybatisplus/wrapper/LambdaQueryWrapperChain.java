...
/**
 * @description: Lambda 查询wrapper
 * @author: Stan | stan.wang@paytm.com
 * @create: 2021/1/26 2:22 下午
 **/
public class LambdaQueryWrapperChain<T>
        extends AbstractChainWrapper<T, SFunction<T, ?>, LambdaQueryWrapperChain<T>, LambdaQueryWrapper<T>>
    ...
    public <R> R getObj(Function<? super Object, R> mapper){
        return baseService.getObj(getWrapper(), mapper);
    }

    public <R> R entity(Function<? super T, R> mapper){
        return baseService.entity(getWrapper(), mapper);
    }
