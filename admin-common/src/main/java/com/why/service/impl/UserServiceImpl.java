...
import com.why.framework.shiro.service.PasswordService;
import com.why.framework.utils.ApiAssert;
...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements UserService {
    ...
    public boolean deleteUserByIds(String ids) {
        ...
        for (Long userId : userIds) {
            ApiAssert.isFalse(AdminResultCode.USER_CANNOT_UPDATE_SUPER_ADMIN, User.isAdmin(userId));
        }
    ...
    public String importUser(List<User> userList, Boolean isUpdateSupport) {
        if (CollectionUtils.isEmpty(userList)) {
            throw new AdminException(HttpServletResponse.SC_BAD_REQUEST, "导入用户数据不能为空！");
        }
        int successNum = 0;
        int failureNum = 0;
        ...
        if (failureNum > 0) {
            ...
            throw new AdminException(HttpServletResponse.SC_BAD_REQUEST, failureMsg.toString());
        } else {
    ...
    public boolean changeStatus(User user) {
        ApiAssert.isFalse(AdminResultCode.USER_CANNOT_UPDATE_SUPER_ADMIN, User.isAdmin(user.getUserId()));
        return updateById(user);
    }
}
